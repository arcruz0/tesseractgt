...
correct_gt_txt <- function(){
  ...
  server <- function(input, output, session) {
    ...
    r_image_files <- shiny::reactive({
      ...
      list.files(input$folder, full.names = TRUE) |>
        stringr::str_subset("\\.(png|tiff{0,1})$")
    })
    ...
    output$textInput <- shiny::renderUI({
      ...
      # Debugging print
      cat("Debug: fpath_text =", fpath_text, "\n")
      
      if (!file.exists(fpath_text)) {
        # Create the .gt.txt file if it doesn't exist
        create_gt_txt(dirname(input$image), r_image_format(), engine = NULL)
      }
      
      file_contents <- try(suppressWarnings(readLines(fpath_text)), silent = TRUE)
      
      # Check if the file can be opened
      if (inherits(file_contents, "try-error")) {
        # Print the error message for debugging
        error_message <- attr(file_contents, "condition")$message
        cat("Error: ", error_message, "\n")
        if (error_message == "cannot open the connection") {
          file_contents <- ""
          writeLines(" ", fpath_text, sep = "\n")
        } else {
          shiny::showNotification(paste("Error reading file:", error_message), type = "error")
          stop("Unknown error")
        }
      }
